...
\title{Primal--Dual Subgradient and Dual Averaging}
\author{Michał Iwaniuk, Mikołaj Guzik}
\date{}
...
\(\delta_k(D)/S_k\) is an upper bound on the objective error of the averaged iterate
\(\hat x_k\).  The radius condition is essential: when
...
The implemented methods are simple dual averaging (SDA), weighted dual
averaging (WDA), stochastic simple averages (SSA), and a projected subgradient
baseline.  The deterministic benchmark experiments use nonsmooth convex objectives with known
...
\(\gamma_{\mathrm{mult}}\) for the dual averaging variants, and, for
deterministic runs, a stopping tolerance \(\varepsilon\).  The boolean option
...
\(\delta_k(D)/S_{k+1}\) & normalized primal--dual certificate gap used for deterministic dual averaging stopping \\
...
for deterministic dual averaging stopping.  It is not the same metric as the
benchmark objective gap or the lasso train-objective gap to SAGA.
...
\subsection{Weighted Dual Averaging (WDA)}

WDA uses the same primal update and the same \(\hat\beta_k\) sequence as SDA,
but changes the aggregation weights to
\[
    \lambda_k=
    \begin{cases}
        1/\|g_k\|_*, & \|g_k\|_*>0,\\
...
In Nesterov's notation, WDA is parameterized by \(\rho\), whereas the
implementation exposes the same \(\gamma\) interface used by SDA.  Since
...
The two larger synthetic datasets are included to stress WDA
...
\]
Small negative lasso train-objective gaps mean that the reported custom iterate
has a lower value of the same train objective than the matched finite-iteration
SAGA run.  They do not prove that the global optimum has been reached, because
the SAGA value is a numerical reference rather than an exact optimum.  SDA also
computes a separate primal--dual certificate gap
...
runtime plots.  When \(D\) is valid, Nesterov's analysis gives
\[
    f(\hat x_k)-f_D^\star \le \frac{\delta_k(D)}{S_k},
    \qquad
    f_D^\star=\min_{x\in F_D} f(x).
\]
If \(D\ge d(x^\star)\), then \(f_D^\star=f^\star\), so the certificate is an
upper bound on the true objective gap.  The converse is not true: a small
objective gap does not imply a small certificate gap.  The certificate is based
on the averaged affine lower model accumulated from all past subgradients, so it
measures a primal--dual model gap rather than only the objective value at
\(\hat x_k\).  This averaged model can remain loose even when the averaged
primal point is already close to optimal, making the certificate-based stopping
rule harder to satisfy than a direct objective-gap target.  The report focuses
on objective gaps because they measure the actual optimization error when a
reference optimum is available and because they are comparable across SDA,
projected subgradient, and SAGA.  The certificate gap remains important as an
SDA stopping rule, but it is method-specific, can be conservative, and can be
misleading when the radius condition fails.  For this reason the experiments use
a fixed \(1000\)-iteration budget when comparing methods and treat
certificate-based early stopping as an additional SDA diagnostic rather than as
the only measure of progress.

The experimental design follows Nesterov's primal--dual subgradient framework:
the method maintains averaged primal iterates and a dual aggregate, while WDA
changes the scale used to aggregate subgradients
\cite{nesterov_primal_dual}.  The comparisons below are empirical.  They
measure objective gaps, runtimes, and sensitivity to \(D\), prox geometry, and
step scaling under the implemented solvers and stored experiment grids.
...
SDA & 432 & \(2.77\cdot 10^{-1}\) & \(3.72\cdot 10^{-4}\) & \(9.35\cdot 10^{-3}\) \\
WDA & 432 & \(1.75\cdot 10^{-1}\) & \(3.35\cdot 10^{-4}\) & \(1.02\cdot 10^{-2}\) \\
\bottomrule
\end{tabular}
}
...
the original problem.  If \(x^\star\in F_D\), then the subgradient inequality gives
\[
...
\]
This yields the SDA stopping condition
\[
...
\[
    \min_{x\in F_D} f(x).
\]
When the iterates are not themselves restricted to \(F_D\), even this
restricted-problem interpretation must be read carefully: \(\hat x_k\) may lie
outside \(F_D\), so the value is not a feasible restricted-solution certificate.
In this regime a negative computed primal--dual certificate gap indicates that
the nonnegativity and upper-bound interpretation of \(\delta_k(D)\) no longer
applies.  Therefore a negative value
\(\delta_k(D)/S_k<\varepsilon\) may trigger early stopping even while the true
objective gap \(f(\hat x_k)-f(x^\star)\) is still large.  Figure~\ref{fig:nonsmooth-unrestricted}
illustrates this behavior.  In the
stored nonsmooth-grid results, \(93\) of the \(864\) SDA runs ended with a
negative final certificate gap while still having true objective gap larger than
\(10^{-3}\).  All \(93\) cases were unrestricted runs with \(D\in\{0.5,2\}\).
The failure is associated with using the certificate outside its radius
assumption, not with the objective-gap metric itself.
...
consistent with the feasible problem being solved, and early stopping caused by
an invalid primal--dual certificate gap is removed.  The price is that the
...
larger values increase the prox regularization and therefore produce smaller
primal moves in response to the observed subgradients, while smaller values
weaken the regularization and produce larger moves.  The figure shows that the
baseline multiplier is not always the best empirical choice in terms of
objective-gap decrease.  The theoretical step is based on worst-case bounds,
while the benchmark objectives have different local geometry, subgradient
magnitudes, and distances from the initial point to the minimizer.  The
experiments indicate that \(\gamma_{\mathrm{mult}}\) should
be treated as a tuning parameter around the theoretically balanced scale, not as
...
baseline scale can become too weak because \(\gamma^\star=L/\sqrt{2D}\)
decreases with \(D\), unless the multiplier is adjusted.  Thus a
...
SDA, WDA, and projected subgradient descent.  On these
...
while the best single run is achieved by WDA.  The methods solve slightly
...
predictions of dominance on every small benchmark.  In these low-dimensional
benchmarks, projected subgradient is a strong baseline, while WDA can be best
after tuning on some individual objectives.
...
and the computed primal--dual certificate gap became invalid or negative.  They
are cases of invalid certificate-based early stopping, as shown in
Figure~\ref{fig:nonsmooth-unrestricted}.  Consequently,
Figure~\ref{fig:nonsmooth-runtime} should be read as a runtime--accuracy plot
with certificate validity checked separately.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{nonsmooth_objective_grid/nonsmooth_method_comparison.pdf}
\caption{Default comparison of SDA, WDA, and projected subgradient descent on nonsmooth benchmark objectives.}
...
\(\ell_1\) objective.  The methods are Euclidean SDA and Euclidean WDA, each
with Euclidean and weighted Euclidean prox choices.  For the weighted Euclidean
...
Method & Runs & Median final objective gap & Best objective gap & Median runtime (s) \\
\midrule
SDA, Euclidean prox & 54 & \(1.36\) & \(1.24\cdot 10^{-2}\) & \(1.45\cdot 10^{-2}\) \\
SDA, weighted prox & 162 & \(2.18\) & \(1.24\cdot 10^{-2}\) & \(1.60\cdot 10^{-2}\) \\
WDA, Euclidean prox & 54 & \(3.14\) & \(1.94\cdot 10^{-2}\) & \(1.51\cdot 10^{-2}\) \\
WDA, weighted prox & 162 & \(2.92\) & \(1.94\cdot 10^{-2}\) & \(1.89\cdot 10^{-2}\) \\
\bottomrule
\end{tabular}
}
...
The results follow the expected role of a prox function: it improves a method
only when it represents the geometry of the feasible set and the
subgradients well; an arbitrary change of norm is not a free acceleration.  The
selected \(D=32\), \(\gamma_{\mathrm{mult}}=1\), unrestricted runs show this
directly: the coordinate-scaled simple weighted prox improves the
ill-conditioned \(\ell_1\) shifted objective relative to the uniform Euclidean
prox, but the inverse-scaled profile is much worse on the same objective.
Weighted geometry should therefore be justified by coordinate scale information,
...
objective.  The median objective gaps in Table~\ref{tab:prox-summary} should be
read together with the per-objective panels rather than as an absolute ranking
of all geometries.  Figure~\ref{fig:prox-runtime} does not support a
runtime-only preference for the weighted prox in this implementation; its value
is conditional on reducing enough iterations or objective gap to offset the
...
\texttt{simplex\_sparse\_max\_affine\_16d}.  Entropy SDA and entropy WDA
...
Method & Runs & Median final objective gap & Best objective gap & Median runtime (s) \\
\midrule
Entropy SDA & 36 & \(8.37\cdot 10^{-3}\) & \(1.09\cdot 10^{-3}\) & \(6.16\cdot 10^{-1}\) \\
Entropy WDA & 36 & \(8.13\cdot 10^{-3}\) & \(1.09\cdot 10^{-3}\) & \(6.57\cdot 10^{-1}\) \\
\bottomrule
\end{tabular}
}
...
Figure~\ref{fig:simplex-entropy} shows that entropy SDA and entropy WDA are
close in median final objective gap.  The best run on
...
objective favors entropy WDA in the best run.  Entropy is appropriate for the
simplex constraint, but WDA is not uniformly dominant on these three
objectives.  The figure also shows the parameter sensitivity observed in the
Euclidean grid: changing \(D\) and \(\gamma_{\mathrm{mult}}\) can matter as much
as changing from simple to weighted averaging.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{simplex_entropy_geometry/simplex_entropy.pdf}
\caption{Entropy SDA and entropy WDA on simplex-constrained objectives.}
...
projection/prox computations.  Entropy prox is geometrically suitable for
simplex constraints, but its practical value must be judged together with
implementation cost.
...
\texttt{spambase}.  The custom methods are SDA, WDA, and
...
SDA & 96 & \(2.03\cdot 10^{-2}\) & \(6.26\cdot 10^{-5}\) & \(4.29\cdot 10^{-2}\) & \(0.878\) \\
WDA & 96 & \(1.74\cdot 10^{-5}\) & \(-1.43\cdot 10^{-3}\) & \(3.87\cdot 10^{-2}\) & \(0.888\) \\
...
Figure~\ref{fig:lasso-d} compares the methods over \(D\).  This figure fixes
\(\gamma_{\mathrm{mult}}=1\) for SDA/WDA and \(\alpha=1\) for projected
subgradient.  WDA is the most reliable method in this lasso grid: it
has the smallest median train-objective gap to the SAGA reference and is the
best custom method on every dataset in the final-run CSV.  This statement
applies to this parameter grid, not as a general theorem.  The lasso objective is
convex but nonsmooth because of the \(\ell_1\) term, and the implementation
handles the penalty with subgradients rather than a specialized proximal
regularized update.
In the stored final-run CSV, the best custom run for each of the \(8\) lasso
datasets uses WDA with \(D=8\); the best
\(\gamma_{\mathrm{mult}}\) varies across datasets from \(0.25\) to \(2\).
Figure~\ref{fig:lasso-d} therefore favors WDA in this grid, but it also shows
that the radius and step scale remain empirical choices.  Table~\ref{tab:lasso-negative-d-sweep}
lists the Figure~\ref{fig:lasso-d} runs where the final train objective is below
the matched SAGA reference.

\begin{table}[htbp]
\centering
\caption{Negative final lasso train-objective gaps in the \(D\)-sweep setting of Figure~\ref{fig:lasso-d}}
\label{tab:lasso-negative-d-sweep}
\smallskip
\small
\resizebox{\textwidth}{!}{%
...
\midrule
\texttt{breast\_cancer\_wdbc.csv} & WDA & \(128\) & \(6.7885\cdot 10^{-2}\) & \(6.8065\cdot 10^{-2}\) & \(-1.80\cdot 10^{-4}\) & \(0.947\) \\
\texttt{spambase.csv} & WDA & \(32\) & \(2.1506\cdot 10^{-1}\) & \(2.1585\cdot 10^{-1}\) & \(-7.94\cdot 10^{-4}\) & \(0.923\) \\
\texttt{spambase.csv} & WDA & \(128\) & \(2.1500\cdot 10^{-1}\) & \(2.1585\cdot 10^{-1}\) & \(-8.47\cdot 10^{-4}\) & \(0.923\) \\
\bottomrule
\end{tabular}
}
\end{table}
...
\caption{Effect of \(D\) on lasso logistic train-objective gap to the sklearn SAGA reference, with \(\gamma_{\mathrm{mult}}=1\) for SDA/WDA and \(\alpha=1\) for projected subgradient.}
\label{fig:lasso-d}
\end{figure}
...
of plotting only differences.  The gap plot uses a signed logarithmic scale
with a zero line, so negative segments indicate train objectives below the
matched SAGA reference.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{lasso_logistic_sparsity/lasso_parameter_sweep.pdf}
\caption{Step-parameter sweep for lasso logistic train-objective gaps at \(D=32\), shown on a signed logarithmic scale with zero marked by a dashed line.}
...
faster on these small and medium lasso datasets.  Among the custom solvers, WDA reaches
...
not as a certificate of global optimality.  The custom WDA implementation is
competitive in train objective within the
custom solver family, while SAGA remains the appropriate optimized baseline for
this finite-sum composite problem.
...
\subsection{Stochastic Simple Averages versus WDA on Synthetic Lasso}
\label{subsec:lasso-ssa}

\paragraph{Purpose and setup.}
The fifth experiment compares full-gradient WDA with stochastic simple
...
Method & Runs & Median train-objective gap to SAGA & Best train-objective gap & Median runtime (s) & Median accuracy \\
\midrule
WDA & 24 & \(6.95\cdot 10^{-6}\) & \(5.40\cdot 10^{-8}\) & \(4.57\cdot 10^{-2}\) & \(0.840\) \\
...
SSA is cheaper per iteration and has a lower median runtime than WDA in
this grid, but its median final train-objective gap is much larger.  WDA is slower
because it evaluates the full training gradient, but on these synthetic
problems it reaches objective values much closer to the SAGA reference.  A
one-sample subgradient has higher variance, so a fixed budget of 1000 iterations
does not match a deterministic full-gradient method unless the cheaper
...
\caption{Runtime versus final train-objective gap for WDA, SSA, and sklearn SAGA on synthetic lasso datasets.}
\label{fig:ssa-runtime}
\end{figure}

Figure~\ref{fig:ssa-param} shows the \(\gamma_{\mathrm{mult}}\) sweep.  The
WDA curves usually descend rapidly toward the SAGA reference, while SSA
...
\caption{Step multiplier sweep for WDA and SSA on synthetic lasso datasets.}
\label{fig:ssa-param}
\end{figure}
...
but does not close the train-objective gap to the same degree as WDA.  Thus, for the
...
\caption{Absolute train objective values for WDA and SSA, with matched sklearn SAGA references.}
\label{fig:ssa-train}
\end{figure}
...
The benchmark and lasso experiments use different reference quantities.  On the
nonsmooth benchmark objectives, the plotted metric is the true objective gap
\(f(\hat x_k)-f^\star\).  On the lasso logistic datasets, the plotted metric is
the train-objective gap \(F(\hat x_k)-F(x_{\mathrm{saga}})\) to the matched
sklearn SAGA run.  These metrics are more useful for comparing the implemented
methods than the primal--dual certificate gap when a reference value is
available.

The primal--dual certificate gap is a valid stopping certificate only under the
radius condition \(D\ge d(x^\star)\).  Under this condition, Nesterov's bound
gives
\[
    f(\hat x_k)-f^\star \le \frac{\delta_k(D)}{S_k}.
\]
Thus the certificate is an upper bound on the direct objective gap, not a
replacement for measuring \(f(\hat x_k)-f^\star\) when \(f^\star\) is known.
The experiments show two practical limitations.  First, if \(D\) is too small,
the certificate can become invalid or negative and may stop the run while the
true objective gap is still large.  Second, even with a valid radius, the
certificate can require many more iterations to reach a tolerance than the
direct objective gap.  For these experiments, the certificate is useful mainly
as a diagnostic for the primal--dual theory and for checking the radius
assumption; it is not the best primary performance metric.

SDA and projected subgradient descent give broadly similar results on several
experiments.  On the nonsmooth objective grid, projected subgradient has the
best median final objective gap, while WDA achieves the best single objective
gap after tuning.  On the lasso logistic grid, SDA improves over projected
subgradient in median train-objective gap, but the main custom-method
improvement comes from WDA.  WDA has the smallest median train-objective gap to
SAGA and is the best custom method on every dataset in the stored final-run
CSV.  This makes WDA useful for the tested lasso logistic objectives, but the
same advantage is not uniform on the nonsmooth benchmark objectives or on the
simplex experiment.

The parameters \(D\) and \(\gamma_{\mathrm{mult}}\) strongly affect all dual
averaging runs.  The experiments do not support a universal default.  \(D\)
must be large enough for the certificate interpretation, and changing \(D\)
also changes the effective theoretical scale
\(\gamma^\star=L/\sqrt{2D}\).  The multiplier \(\gamma_{\mathrm{mult}}\)
therefore needs to be tuned together with \(D\), especially when objective-gap
performance rather than certificate satisfaction is the goal.

Geometry helps only when it matches the problem structure.  Weighted Euclidean
prox can improve behavior on coordinate-imbalanced objectives when the weights
match the coordinate scales, but mismatched weights can worsen the final gap.
Entropy prox is appropriate for simplex-constrained objectives and gives stable
results there, but the simple and weighted entropy variants have similar median
final objective gaps in the tested grid.

The stochastic simple averages experiment shows the cost--accuracy trade-off of
using sampled subgradients.  SSA is cheaper per iteration and has lower median
runtime in the synthetic lasso comparison, but its final train-objective gaps
remain much larger than those of full-gradient WDA within the same
\(1000\)-iteration budget.  For the current implementation, stochastic sampling
is useful for reducing per-iteration cost, but it does not replace full-gradient
WDA when the target is a small train-objective gap.

...
METHOD_ORDER = ["sda_simple_entropy", "sda_weighted_entropy"]
METHOD_LABELS = {
    "sda_simple_entropy": "SDA simple, entropy",
    "sda_weighted_entropy": "SDA weighted, entropy",
}
METHOD_COLORS = {
    "sda_simple_entropy": "#2563eb",
    "sda_weighted_entropy": "#dc2626",
}
DASH_BY_D_INDEX = ["solid", "dash", "dot", "dashdot", "longdash", "longdashdot"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create Plotly diagnostics for simplex_entropy_geometry results."
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Dual averaging mode filter. Repeat to include multiple. Default: all.",
    )
    parser.add_argument(
        "--D",
        dest="d_values",
        ...
        type=float,
        default=[],
        help="Entropy radius D to include. Repeat to include multiple values. Default: all.",
    )
    parser.add_argument(
        "--metric",
...
def main() -> None:
    ...
    runs = filter_runs(
        ...
        dual_averaging=args.dual_averaging,
...
def add_plot_metadata(runs: pd.DataFrame) -> pd.DataFrame:
    ...
    d_values = sorted(runs["D"].dropna().unique())
    runs["D_dash"] = runs["D"].map(
        {value: DASH_BY_D_INDEX[index % len(DASH_BY_D_INDEX)] for index, value in enumerate(d_values)}
    )
...
def filter_runs(
    ...
    *,
    ...
    if dual_averaging:
        filtered = filtered[filtered["dual_averaging"].isin(dual_averaging)].copy()
    if d_values:
        filtered = filtered[filtered["D"].isin(d_values)].copy()
    return filtered
...
def write_trajectory_figure(
    ...
    *,
    ...
    fig = trajectory_figure(
        data,
        metrics=metrics,
        title=f"{objective_id}: entropy SDA objective trajectories",
                ...
                fig.add_trace(
                    go.Scattergl(
                        ...
                        hovertemplate=(
                            ...
                            "objective_gap=%{meta[6]:.6g}<br>"
                            "certificate_gap=%{meta[7]:.6g}"
                            "<extra></extra>"
                        ),
                    )
                )
...
def write_runtime_objective_gap_scatter(runs: pd.DataFrame, out_dir: Path) -> Path:
    fig = px.scatter(
        ...
        title="Runtime vs final objective gap for entropy SDA",
        labels={
            "runtime_seconds": "Runtime (seconds)",
            "objective_gap": "Final objective gap",
            "method_label": "Method",
            "D_label": "Entropy radius",
